...
class Graph:
    ...
    def incoming_edge_indices(self, vertices):
        ...
        if not self.directed:
            raise ValueError(
                "Incoming edge indices cannot be computed for undirected graphs."
            )
    ...
    def outgoing_edge_indices(self, vertices):
        ...
        if not self.directed:
            raise ValueError(
                "Outgoing edge indices cannot be computed for undirected graphs."
            )
...
class GraphOfConicSets(Graph):
    ...
    def solve_traveling_salesman(
        self, subtour_elimination=True, binary=True, tol=1e-4, **kwargs
    ):
    ...
    def solve_minimum_spanning_tree(
        self, root=None, subtour_elimination=True, binary=True, tol=1e-4, **kwargs
    ):
        """
        Parameter root is ignored for undirected graphs.
        """
        return minimum_spanning_tree(
            self, root, subtour_elimination, binary, tol, **kwargs
        )
...
class GraphOfConvexSets(Graph):
    ...
    def _set_solution(self, conic_graph):
        ...
        for convex_vertex, conic_vertex in zip(self.vertices, conic_graph.vertices):
            if conic_vertex.x.value is None:
                ...
                for variable in convex_vertex.variables:
                    variable.value = conic_vertex.get_convex_variable_value(
                        variable, conic_vertex.x.value
                    )
        ...
        for convex_edge, conic_edge in zip(self.edges, conic_graph.edges):
            if conic_edge.x.value is None:
                ...
                for variable in convex_edge.variables:
                    variable.value = conic_edge.get_convex_variable_value(
                        variable, conic_edge.x.value
                    )
    ...
    def solve_shortest_path(self, source, target, binary=True, tol=1e-4, **kwargs):
        ...
        ret = conic_graph.solve_shortest_path(
            conic_source, conic_target, binary, tol, **kwargs
        )
        self._set_solution(conic_graph)
        return conic_graph, *ret
    ...
    def solve_minimum_spanning_tree(
        ...
        conic_graph.solve_minimum_spanning_tree(
            conic_root, subtour_elimination, binary, tol, **kwargs
        )
        self._set_solution(conic_graph)
    ...
    def solve_shortest_path_with_rounding(
        self, source, target, rounding_fn, tol=1e-4, **kwargs
    ):
        binary = False
